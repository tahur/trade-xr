...
This module provides secure storage for Zerodha Kite API credentials
using the BYOK (Bring Your Own Key) model:

- Uses machine-derived key (same as session storage) — no user password needed
...
class CredentialVault:
    """Encrypt and decrypt API credentials with machine-derived key."""
    ...
    def save(api_key: str, api_secret: str) -> None:
        """Encrypt and save credentials to vault file using machine key."""
    ...
    def load() -> Dict[str, str]:
        """Decrypt and load credentials from vault file using machine key."""
        vault_path = Path(__file__).parent.parent.parent / CredentialVault.VAULT_FILE
