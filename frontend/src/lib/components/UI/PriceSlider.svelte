    ...
    $: {
        ...
        if (isOrderWindowOpen) {
            ...
            if ($gestureState.detectedGesture === 'Victory') {
                console.log(`[ORDER] Placed Limit Order: Price ${confirmedPrice}, Qty ${$gestureState.fingerCount}`);
                orderPlaced = true;
    ...
    <div class="fixed inset-0 pointer-events-none z-50">
        ...
        >
            ...
            >
                ...
                <div class="mt-2 text-[10px] uppercase font-bold text-emerald-600 flex items-center justify-center gap-1 animate-pulse">
        ...
        {#if isOrderWindowOpen}
            <div class="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60]" transition:fade>
                <div class="bg-white rounded-xl shadow-2xl p-8 max-w-sm w-full text-center border-4 border-emerald-500/30">
                    <h2 class="text-sm font-bold text-zinc-500 uppercase tracking-widest mb-4">Place Limit Order</h2>

                    <div class="mb-6">
                        <div class="text-xs text-zinc-400 mb-1">PRICE</div>
                        <div class="text-4xl font-mono font-bold text-zinc-800">{(confirmedPrice ?? 0).toFixed(2)}</div>
                    </div>

                    <div class="mb-8 p-4 bg-emerald-50 rounded-lg border border-emerald-100">
                        <div class="text-xs font-bold text-emerald-600 uppercase tracking-wide mb-2">QUANTITY</div>
                        <div class="text-6xl font-bold text-emerald-600">
                            {Math.min($gestureState.fingerCount, 10)}
                        </div>
                        <div class="text-[10px] text-emerald-400 mt-2 font-bold uppercase">Showing {$gestureState.fingerCount} Fingers (Max 10)</div>
                    </div>

                    <div class="flex justify-between items-center text-[10px] font-bold uppercase text-zinc-400 tracking-wide border-t pt-4 border-zinc-100">
                        <div class="flex items-center gap-1"><span class="text-xl">✊</span> CANCEL</div>
                        <div class="flex items-center gap-1"><span class="text-xl">✌️</span> CONFIRM</div>
                    </div>
                </div>
            </div>
        {/if}
        ...
        {#if orderPlaced}
             <div class="absolute inset-0 flex items-center justify-center z-[70]">
                <div class="bg-emerald-500 text-white px-8 py-6 rounded-2xl shadow-[0_20px_60px_rgba(16,185,129,0.4)] text-center transform scale-125">
                    <div class="text-6xl mb-2">🚀</div>
                    <div class="text-2xl font-bold tracking-tight">ORDER PLACED!</div>
                    <div class="text-emerald-100 font-mono mt-1">{(confirmedPrice ?? 0).toFixed(2)} x {Math.min($gestureState.fingerCount, 10)}</div>
                </div>
            </div>
        {/if}
